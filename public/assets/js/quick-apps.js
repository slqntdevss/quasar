import apps from "../json/apps.json";

const iconUrls = import.meta.glob(
	"../img/{ddg,youtube,tiktok,reddit,twitter,movies}.webp",
	{ eager: true, query: "?url", import: "default" },
);

function resolveIcon(imgPath) {
	const name = imgPath.replace(/^\//, "");
	const key = `../img/${name}`;
	return iconUrls[key] || `/assets/img/${name}`;
}

(function () {
	const grid = document.getElementById("quick-apps-grid");
	if (!grid) return;
	try {
		grid.innerHTML = `<div class="apps-grid-container">
		${apps
			.slice(0, 6)
			.map(
				(app) => `
			<div class="apps-grid-tile" tabindex="0" data-url="${app.url}">
				<img src="${resolveIcon(app.img)}" alt="${app.name}" />
				<span class="apps-grid-label">${app.name}</span>
			</div>
		`,
			)
			.join("")}
		</div>`;
	} catch (e) {
		grid.innerHTML =
			'<div style="color: var(--main-text, white); text-align:center;">Error loading apps.</div>';
	}
})();
