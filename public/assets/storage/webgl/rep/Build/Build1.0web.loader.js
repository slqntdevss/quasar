		...
		l.preRun.push(function () {
			(l.addRunDependency("dataUrl"),
				o.then(function (e) {
					...
					// Check if we can read 4 bytes at this position
					if (n + 4 > t.byteLength) {
						console.error(
							`Offset overflow: tried to read at ${n}, buffer size is ${t.byteLength}`,
						);
						console.error(
							`r.length was ${r.length}, previous offset was ${oldN}`,
						);
